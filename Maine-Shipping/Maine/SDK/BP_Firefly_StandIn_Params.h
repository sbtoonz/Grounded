#pragma once

/**
 * Name: Maine
 * Version: Shipping
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function BP_Firefly_StandIn.BP_Firefly_StandIn_C.ReceiveBeginPlay
	 */
	struct ABP_Firefly_StandIn_C_ReceiveBeginPlay_Params
	{	};

	/**
	 * Function BP_Firefly_StandIn.BP_Firefly_StandIn_C.ToggleVFX
	 */
	struct ABP_Firefly_StandIn_C_ToggleVFX_Params
	{
	public:
		int32_t                                                    NewHour;                                                 // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
		int32_t                                                    NewDay;                                                  // 0x0004(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function BP_Firefly_StandIn.BP_Firefly_StandIn_C.ExecuteUbergraph_BP_Firefly_StandIn
	 */
	struct ABP_Firefly_StandIn_C_ExecuteUbergraph_BP_Firefly_StandIn_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
		unsigned char                                              UnknownData_R58F[0x4];                                   // 0x0004(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY) ()
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
